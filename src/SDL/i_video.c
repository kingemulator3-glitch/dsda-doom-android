    ...
    switch (Event->type) {
      ...
      case SDL_CONTROLLERBUTTONUP:
        ...
        break;
             ...
             case SDL_FINGERDOWN:
        if (Event->tfinger.x < 0.25 && Event->tfinger.y > 0.70)
        {
          event.type = ev_keydown;
          event.data1.i = KEYD_ENTER;
          D_PostEvent(&event);
        }
        else if (Event->tfinger.x > 0.70 && Event->tfinger.y > 0.70)
        {
          event.type = ev_joystick;
          event.data1.i = 1 << DSDA_CONTROLLER_BUTTON_TRIGGERRIGHT;
          D_PostEvent(&event);
        }
        else if (Event->tfinger.x < 0.5)
        {
          event.type = ev_move_analog;
          event.data1.f = 0.0f;
          event.data2.f = -1.0f;
          D_PostEvent(&event);
        }
...
void I_FinishUpdate (void)
{
  ...
  // Draw!
  SDL_SetRenderDrawColor(sdl_renderer, 255, 255, 255, 120);

  SDL_Rect fire_button = {SCREENWIDTH - 80, ACTUALHEIGHT - 80, 60, 60};

  SDL_RenderFillRect(sdl_renderer, &fire_button);
  SDL_Rect start_button = {20, ACTUALHEIGHT - 80, 60, 60};
  SDL_RenderFillRect(sdl_renderer, &start_button);
